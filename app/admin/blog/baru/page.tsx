...
export default function TulisBaru() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const slug = form.slug || generateSlug(form.judul);
    await supabase
      .from("posts")
      .insert([{ ...form, slug, published_at: new Date().toISOString() }]);
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="text-3xl font-serif font-bold mb-6">Tulis Artikel Baru</h1>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <textarea
            rows={12}
            className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-stone-500"
            value={form.konten}
